...
procedure Hashmap_Example is
   ...
   function Hash_Func
     (Key : Ada.Strings.Unbounded.Unbounded_String) return Ada.Containers
     .Hash_Type
   is
   begin
   ...
   package Attendance_Map is new Ada.Containers.Hashed_Maps
     (Key_Type        => Ada.Strings.Unbounded.Unbounded_String,
      Equivalent_Keys => Equivalent_Keys, Hash => Hash_Func,
   ...
   procedure Print_Hash_Map (Position : Attendance_Map.Cursor) is
   begin
   ...
   loop
      ...
      case User_Input is
         when 1 =>
            ...
            if To_Lower (Confirmation) =
              Ada.Strings.Unbounded.To_Unbounded_String ("n") or
              To_Lower (Confirmation) =
                Ada.Strings.Unbounded.To_Unbounded_String ("no")
            then
               Attendance_Map.Insert
                 (Container => Wedding_Attendance, Key => String_Input,
                  New_Item  => False);
            elsif To_Lower (Confirmation) =
              Ada.Strings.Unbounded.To_Unbounded_String ("y") or
              To_Lower (Confirmation) =
         ...
         when 2 =>
            ...
            exception
               when Constraint_Error =>
                  Ada.Text_IO.Put_Line
                    ("Name " & Ada.Strings.Unbounded.To_String (String_Input) &
