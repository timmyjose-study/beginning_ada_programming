...
procedure Vector_Of_Names is
  ...
  procedure Populate_Names ( Vec: in out Names_Vector_Tracker.Vector ) is
    Continue: Ada.Strings.Unbounded.Unbounded_String := Ada.Strings.Unbounded.Null_Unbounded_String;
  begin
    loop
  ...
  begin
    for Idx in 1 .. Vec.Length loop
      Ada.Text_IO.Unbounded_IO.Put_Line (Vec.Element (Index => Natural (Idx)));
    end loop;
  end Display_Names;
begin
