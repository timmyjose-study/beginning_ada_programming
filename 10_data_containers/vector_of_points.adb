...
procedure Vector_Of_Points is
   ...
   procedure Deallocate is new Ada.Unchecked_Deallocation (Point,
      Point_Access);
   procedure Deallocate is new Ada.Unchecked_Deallocation (Point_Integer,
      Point_Integer_Access);

   package Point_Vector is new Ada.Containers.Vectors (Index_Type => Positive,
   ...
   begin
      ...
      for Iter in 1 .. Num_Elements loop
         ...
         begin
            ...
            P_Acc :=
              new Point'
                ((X => new Point_Integer'(X), Y => new Point_Integer'(Y)));
   ...
   begin
      for Idx in 1 .. Points.Length loop
         ...
         begin
            Ada.Text_IO.Put
              ("Point { X = " & Integer'Image (P.all.X.all) & ", Y = " &
               Integer'Image (P.all.Y.all) & " }");
         end;
      end loop;
   end Display_Points;
